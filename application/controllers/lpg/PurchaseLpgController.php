...
//        exit;
    ...
    public function purchases_lpg_edit($ivnoiceId = null)
    {
        ...
        if (isPostBack()) {
            ...
            if (!empty($_POST['slNo'])) {
                foreach ($_POST['slNo'] as $key => $value) {
                    ...
                    $stockNew['property_1'] = $_POST['property_1_' . $value];
                    $stockNew['property_2'] = $_POST['property_2_' . $value];
                    $stockNew['property_3'] = $_POST['property_3_' . $value];
                    $stockNew['property_4'] = $_POST['property_4_' . $value];
                    $stockNew['property_5'] = $_POST['property_5_' . $value];
        ...
        foreach ($stockList as $ind => $element) {
            ...
            $result[$element->purchase_invoice_id][$element->purchase_invoice_id]['tt_returnable_quantity'] = $element->tt_returnable_quantity;
            $result[$element->purchase_invoice_id][$element->purchase_invoice_id]['property_1'] = $element->property_1;
            $result[$element->purchase_invoice_id][$element->purchase_invoice_id]['property_2'] = $element->property_2;
            $result[$element->purchase_invoice_id][$element->purchase_invoice_id]['property_3'] = $element->property_3;
            $result[$element->purchase_invoice_id][$element->purchase_invoice_id]['property_4'] = $element->property_4;
            $result[$element->purchase_invoice_id][$element->purchase_invoice_id]['property_5'] = $element->property_5;
...
//pending check condition
    ...
    public function purchase_delete(){
        ...
        if(!empty($customer_money_revcive)){
            $msg = 'Purchase Invoice Cannot Delete .This Sales Invoice have accounting transaction  ' ;
            $this->session->set_flashdata('error', $msg);
            redirect(site_url($this->project . '/salesInvoiceLpg'));
        }*/
        $this->db->trans_begin();
        $purchase_inv=array();
        $purchase_inv['is_active'] = 'N';
        $purchase_inv['is_delete'] = 'Y';
    ...
    public function _save_data_to_purchase_history_table($invoice_id, $action = array('edit', 'delete'))
    {
        ...
        // $this->db->trans_begin();
        $sql_sales_invoice_info_audit = "create table IF NOT EXISTS purchase_invoice_info_audit(purchase_invoice_info_history_id int not null auto_increment, PRIMARY KEY (sales_invoice_info_history_id)) as select * from sales_invoice_info where 1=3";
        $this->db->query($sql_sales_invoice_info_audit);
        $sql_sales_details_audit = "create table IF NOT EXISTS purchase_details_audit(purchase_invoice_info_history_id int not null ) as select * from sales_details where 1=3";
        $this->db->query($sql_sales_details_audit);
        $sql_sales_return_details_audit = "create table IF NOT EXISTS purchase_return_details_audit(purchase_invoice_info_history_id int not null ) as select * from sales_return_details where 1=3";
        ...
        $sales_invoice_info_old_array = $this->Common_model->get_data_list_by_many_columns_array('purchase_invoice_info', $sales_invoice_info_old_condition);
        if ($action == 'edit') {
        ...
        $sales_invoice_info_history_id = $this->Common_model->insert_data('purchase_invoice_info_audit', $sales_invoice_info_old_array[0]);
        ...
        $sales_details_old_array = $this->Common_model->get_data_list_by_many_columns_array('purchase_details', $sales_details_old_condition);

        foreach ($sales_details_old_array as $key => $csm) {
            if ($action == 'edit') {
                $sales_details_old_array[$key]['purchase_invoice_info_history_id'] = $sales_invoice_info_history_id;
        ...
        $this->Common_model->insert_batch_save('purchase_details_audit', $sales_details_old_array);
        ...
        foreach ($sales_return_details_old_array as $key => $csm) {
            if ($action == 'edit') {
                $sales_return_details_old_array[$key]['purchase_invoice_info_history_id'] = $sales_invoice_info_history_id;
        ...
        $this->Common_model->insert_batch_save('purchase_return_details_audit', $sales_return_details_old_array);
