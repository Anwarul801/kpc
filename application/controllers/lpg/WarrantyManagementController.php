...
class WarrantyManagementController extends CI_Controller
{
    ...
    public function warranty_claim_voucher($confirmId = null)
    {
        ...
        if (isPostBack()) {
            ...
            if ($this->form_validation->run() == FALSE) {
               ...
               // $totalGR_DEBIT = $totalGR_DEBIT + $totalProductCost;
                ...
                log_message('error','$totalGR_DEBIT'.print_r($totalGR_DEBIT,true));
                log_message('error','$totalGR_CREDIT'.print_r($totalGR_CREDIT,true));
        ...
        $menu  = check_parmission_by_user_role(125);
        if($add == 0 || $menu == 0){
    ...
    public function warranty_receipt_voucher($confirId = null)
    {
        ...
        $this->load->helper('site_helper');
        $add  = check_parmission_by_user_role(2116);
        $menu  = check_parmission_by_user_role(126);
        if($add == 0 || $menu == 0){
            $data['mainContent'] = $this->load->view('distributor/not_permisson_page', $data, true);
