...
//52 Purchase price Variance
    ...
    function suppay_ajax()
    {
        ...
        foreach ($query['invoice_list'] as $key => $row):
            if ($row['supplier_invoice_no'] != '') {
                $invoice_no = $row['invoice_no'] . ' (' . $row['supplier_invoice_no'] . ' )';
            }else{
            ...
            $result .= '<tr>';
            $result .= '<td class="text-center"><a href="' . site_url($this->project . $route . $row['purchase_invoice_id']) . '" target="_blank">' . $invoice_no . '<input type="hidden" name="voucher[]" value="' . $invoice_no . '"></a></td>';
    ...
    public function supplierPaymentAdd()
    {
        if (isPostBack()) {
          ...
          //when bank paymet transaction s from here
                    ...
                    $mreceit = array(
                        ...
                        'invoiceID'=>implode(",",$_POST['voucher']),
                        'receitID' => $mrid,
        ...
        $menu  = check_parmission_by_user_role(37);
