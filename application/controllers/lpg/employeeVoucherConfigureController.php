...
class employeeVoucherConfigureController extends CI_Controller
{
    private $timestamp;
    private $admin_id;
    public $dist_id;
    ...
    public function voucherConfiquration()
    {
       ...
       if (isPostBack()) {
                ...
                $this->db->update('salary_bonus_field', $Condition);
                 
                    foreach ($_POST['fieldName'] as $key => $value) {
                        unset($data);
                       $get_id = $this->Common_model->get_single_data_by_single_column('salary_bonus_field', 'id', $value);
                ...
                $this->db->update_batch('salary_bonus_field', $allAccess,'id');
                $this->db->trans_complete();
                if ($this->db->trans_status() === FALSE) {
                    $msg = "Your data can't be update";
                    $this->session->set_flashdata('error', $msg);
                    redirect(site_url($this->project . '/voucherConfiquration'));
                } else {
        ...
        $data['EmployeeField'] = $this->Common_model->get_data_list('salary_bonus_field', 'id', 'ASC');
        $data['Employeevoucher'] = $this->MyModel->getSalaryVoucherField();
       // $data['dayBookCofig'] = $this->Accounts_model->getalldayBook();

        $data['title'] = 'Employee Voucher Confiquration';
        $data['page_type'] = $this->page_type;
        $data['link_page_name'] = '';
        $data['link_page_url'] = '';
        $data['link_icon'] = "";
        /*page navbar details*/
        $data['pageTitle'] = 'Employee Voucher Confiquration';
        $data['mainContent'] = $this->load->view('distributor/setup/employee/voucherConfiguration', $data, true);
        $this->load->view('distributor/masterTemplate', $data);
    }
   
    public function voucherConfigEdit($editId){
         if (isPostBack()) {
           ...
           $this->form_validation->set_rules('field', 'field', 'required');
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                $this->Common_model->update_data('salary_bonus_field', $data, 'id', $editId);
                $this->db->trans_complete();
                if ($this->db->trans_status() === FALSE) {
        ...
        $data['getSingleModel'] = $this->MyModel->getSingleBonusField($editId);
        /*page navbar details*/
        $data['title'] = get_phrase('Voucher Confiquration');
        $data['page_type'] = $this->page_type;
        $data['link_page_name'] = get_phrase('Confiquration');
        $data['link_page_url'] = $this->project.'/voucherConfiquration';

        $data['link_icon'] = "<i class='fa fa-list'></i>";
        /*page navbar details*/
        $data['mainContent'] = $this->load->view('distributor/setup/employee/voucherConfigEdit', $data, true);
        $this->load->view('distributor/masterTemplate', $data);
