...
class FestivalBonusController extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('FestivalBonus_Model');
 ...
 public function addFestivalBonus(){
  ...
  if (isPostBack()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                redirect(site_url($this->project . '/addFestivalBonus/'));
            } else {
    ...
    // exit();
                ...
                if ($this->db->trans_status() === FALSE) {
                    $msg = 'Festival Bonus ' . ' ' . $this->config->item("save_error_message");
                    $this->session->set_flashdata('error', $msg);
                    redirect(site_url($this->project . '/addFestivalBonus'));
                } else {
                    $msg = 'Festival Bonus Save ' . ' ' . $this->config->item("save_success_message");
        ...
        $data['title'] = 'Add Festival Bonus';

        /*page navbar details*/
        $data['title'] = get_phrase('Add Festival Bonus');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('Festival Bonus List');
        $data['link_page_url'] = $this->project.'/festivalVoucherList';
        ...
        $data['percentanceSum'] = $this->FestivalBonus_Model->getpercentanceSum();
        $data['percentanceSubt'] = $this->FestivalBonus_Model->getpercentanceSubt();
    //      echo "<pre>";
    //    echo $this->db->last_query();
    ...
    // exit();
        ...
        $data['mainContent'] = $this->load->view('distributor/setup/employee/addFestivalBonus', $data, true);
        $this->load->view('distributor/masterTemplate', $data);


    }
    public function festivalVoucherEdit($voucher) {
  ...
  if (isPostBack()) {
          ...
          // exit();
           ...
           $this->form_validation->set_rules('month', 'Month Name', 'required');
            ...
            if ($this->form_validation->run() == FALSE) {
                $msg = 'Required field can not be Empty ';
                $this->session->set_flashdata('error', $msg);
                redirect(site_url($this->project . '/festivalVoucherList/'));
            } else {

                $this->db->trans_start();

                   $this->FestivalBonus_Model->festivalVoucherDelete($voucher);
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    $msg = 'Festival Bonus ' . ' ' . $this->config->item("save_success_message");
                    $this->session->set_flashdata('success', $msg);
                    redirect(site_url($this->project . '/festivalVoucherList'));
                }
            }
        ...
        /*page navbar details*/
        $data['title'] = get_phrase('Festival Bonus');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('Festival Bonus Voucher List');
        $data['link_page_url'] = $this->project.'/festivalVoucherList';
        $data['link_icon'] = "<i class='fa fa-plus'></i>";
        /*$data['second_link_page_name'] = get_phrase('Invoice_List');
        $data['second_link_page_url'] = 'salesInvoiceLpg';
        ...
        $data['third_link_icon'] = $this->link_icon_edit;*/
        /*page navbar details*/
         $data['getAllsalaryByDateListView'] = $this->FestivalBonus_Model->getAllsalaryByVoucher($voucher);
       //         echo '<pre>';
       // echo $this->db->last_query();
        ...
        $data['employee'] = $this->db->where('empStatus', 'Active')->where('isDelete', 'N')->get('employee')->result();
        $data['employee2'] = $this->Common_model->get_employee();
        $data['mainContent'] = $this->load->view('distributor/setup/employee/festivalBonusEdit', $data, true);
        $this->load->view('distributor/masterTemplate', $data);
    }
     public function festivalVoucherView($voucher){
        $data['title'] = get_phrase('Festival Bonus');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('Add Festival Bonus');
        $data['link_page_url'] = $this->project.'/addFestivalBonus';
        $data['link_icon'] = "<i class='fa fa-plus'></i>";
        $data['third_link_page_name'] = get_phrase('Configuration');
        $data['third_link_page_url'] = $this->project.'/voucherConfiquration';
        $data['third_link_icon'] = "<i class='fa fa-plus'></i>";
        ...
        //   exit();
        $data['mainContent'] = $this->load->view('distributor/setup/employee/festivalBonusView', $data, true);
        $this->load->view('distributor/masterTemplate', $data);

    }

     public function festivalVoucherList() {
        $condition = array(
            'isActive' => 'Y',
            'isDelete' => 'N',
        );
        ...
        /*page navbar details*/
        $data['title'] = get_phrase('Festival Bonus Voucher List');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('Add Festival Bonus');
        $data['link_page_url'] = $this->project.'/addFestivalBonus';
        $data['link_icon'] = "<i class='fa fa-plus'></i>";
       /* $data['second_link_page_name'] = get_phrase('Invoice_List');
        ...
        $data['getAllfestivalBonusDateList'] = $this->FestivalBonus_Model->getAllfestivalBonusDateList();

        $data['employeewisedep'] = $this->Common_model->getAllEmployeewiseD();
        $data['mainContent'] = $this->load->view('distributor/setup/employee/festivalbonusList', $data, true);
        $this->load->view('distributor/masterTemplate', $data);
    }
   
    public function festivalVoucherDelete($voucher){




         $result= $this->FestivalBonus_Model->festivalVoucherDelete($voucher);
      // $result= $this->Common_model->delete_data('salary_info',$condition);

        if ($result) {
            $msg = 'Festival Bonus  ' . ' ' . $this->config->item("delete_success_message");
            $this->session->set_flashdata('success', $msg);
            redirect(site_url($this->project . '/festivalVoucherList'));

        } else {
           $msg = 'Festival Bonus  ' . ' ' . $this->config->item("delete_error_message");
            $this->session->set_flashdata('error', $msg);
            redirect(site_url($this->project . '/festivalVoucherList'));
        }

    }
      public function festivalVoucherReport(){
        $data['title'] = get_phrase('Festival Bonus Report');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('Festival Bonus');
        $data['link_page_url'] = $this->project.'/festivalVoucherList';
        $data['link_icon'] = "<i class='fa fa-plus'></i>";
        ...
        $data['employeefield'] = $this->db->where('isShow', '1')->get('salary_bonus_field')->result();
        $data['getAllsalaryByDateListView'] = $this->FestivalBonus_Model->getAllFestivalVoucherReport();
        $data['mainContent'] = $this->load->view('distributor/setup/employee/report/festivalVoucherReport', $data, true);
        $this->load->view('distributor/masterTemplate', $data);
