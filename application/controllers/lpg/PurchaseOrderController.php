...
class PurchaseOrderController extends CI_Controller
{
    ...
    public function purchase_order_add()
    {
        ...
        if (isPostBack()) {
            ...
            $this->form_validation->set_rules('purchase_order_date', 'PO Date', 'required');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $po_date = $this->input->post('purchase_order_date') != '' ? date('Y-m-d', strtotime($this->input->post('purchase_order_date'))) : 'NULL';
                $delivery_date = $this->input->post('po_delivery_date') != '' ? date('Y-m-d', strtotime($this->input->post('po_delivery_date'))) : 'NULL';
                ...
                $po_info['form_id'] = 8;
                $po_info['so_po_no'] = $this->input->post('po_id');
                $po_info['customer_id'] = 0;
                $po_info['supplier_id'] = $supplier_id;

                $po_info['refference_invoice_no'] = $this->input->post('userInvoiceId');
                $po_info['shipping_address'] = $shippingAddress;
                $po_info['delivery_date'] = $delivery_date;
                $po_info['so_po_date'] = $po_date;
                $po_info['refference_person_id'] = $this->input->post('reference');
                $po_info['narration'] = $narration;
                $po_info['status'] = 0;//1->complete
                $po_info['company_id'] = $this->dist_id;

                $po_info['branch_id'] = $branch_id;
                $po_info['insert_date'] = $this->timestamp;
                $po_info['insert_by'] = $this->admin_id;
                $po_info['update_by'] = '';
                $po_info['update_date'] = '';
                $po_info['delete_by'] = '';
                $po_info['delete_date'] = '';
                ...
                $po_info['extra_field_1'] = $this->input->post('purchase_order_extra_field_1');
                $po_info['extra_field_2'] = $this->input->post('purchase_order_extra_field_2');
                $po_info['extra_field_3'] = $this->input->post('purchase_order_extra_field_3');
                $po_info['extra_field_4'] = $this->input->post('purchase_order_extra_field_4');
                $po_info['extra_field_5'] = $this->input->post('purchase_order_extra_field_5');

                $po_info['lc_field_1'] = $this->input->post('purchase_order_lc_field_1');
                $po_info['lc_field_2'] = $this->input->post('purchase_order_lc_field_2');
                $po_info['lc_field_3'] = $this->input->post('purchase_order_lc_field_3');


                $po_id = $this->Common_model->insert_data('so_po_info', $po_info);
        ...
        $data['title'] = get_phrase('Purchase Order Add');
        $data['page_type'] = get_phrase($this->page_type);
        $data['second_link_page_name'] = get_phrase('Purchase Order list');
        ...
        if($add == 0 || $menu == 0){
            ...
            $data['mainContent'] = $this->load->view('distributor/purchase_order/purchase_order_add', $data, true);
    ...
    public function purchase_order_edit($id)
    {
        ...
        if (isPostBack()) {
            ...
            $this->form_validation->set_rules('purchase_order_date', 'So Date', 'required');
            $this->form_validation->set_rules('supplier_id', 'Supplier ID', 'required');
            $this->form_validation->set_rules('branch_id', 'Branch  ID', 'required');
            $this->form_validation->set_rules('slNo[]', 'Voucehr ID', 'required');
            $this->form_validation->set_rules('price[]', 'Product Price', 'required');
            if ($this->form_validation->run() == FALSE) {
                ...
                $branch_id = $this->input->post('branch_id');
                $supplier_id = $this->input->post('supplier_id');
                $purchase_order_date = $this->input->post('purchase_order_date') != '' ? date('Y-m-d', strtotime($this->input->post('purchase_order_date'))) : 'NULL';
                $delivery_date = $this->input->post('po_delivery_date') != '' ? date('Y-m-d', strtotime($this->input->post('po_delivery_date'))) : 'NULL';
                $narration = $this->input->post('narration');
                $shippingAddress = $this->input->post('shippingAddress');
                ...
                $po_info['shipping_address'] = $shippingAddress;
                $po_info['delivery_date'] = $delivery_date;
                $po_info['so_po_date'] = $purchase_order_date;
                $po_info['refference_person_id'] = $this->input->post('reference');
                $po_info['narration'] = $narration;
                $po_info['status'] = 0;//1->complete
                $po_info['company_id'] = $this->dist_id;
                ...
                $this->Common_model->save_and_check('so_po_info', $po_info, $so_po_info_update_condition);


                $this->_save_data_to_so_po_history_table($id, $voucherType = 8, $action = 'edit', $redirect_page = 'purchase_order_edit', $name);
                ...
                foreach ($_POST['slNo'] as $key => $value) {
                    ...
                    $so_product['form_id'] = 8;
                    $so_product['delivery_date'] = $delivery_date;
                    $so_product['so_po_date'] = $purchase_order_date;
                    $so_product['product_id'] = $product_id;
                    $so_product['so_po_qty'] = $quantity;
                    $so_product['so_po_unit_price'] = $unit_price;
                    $so_product['so_po_approve_qty'] = 0;
                ...
                if ($this->db->trans_status() === FALSE) {
                    $msg = 'Purchase Order ' . ' ' . $this->config->item("save_error_message");
                    $this->session->set_flashdata('error', $msg);
                    redirect(site_url($this->project . '/purchase_order_list/'));
                } else {
                    $msg = 'Purchase Order  ' . ' ' . $this->config->item("save_success_message");
                    $this->session->set_flashdata('success', $msg);
                    redirect(site_url($this->project . '/purchase_order_list/'));
        ...
        $data['title'] = get_phrase('Purchase_order_edit');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('New_Purchase_Order');
        $data['link_page_url'] = $this->project . '/purchase_order';
        $data['link_icon'] = $this->link_icon_add;
        $data['second_link_page_name'] = get_phrase('Purchase_order_list');
        $data['second_link_page_url'] = $this->project . '/purchase_order_list';
        $data['second_link_icon'] = $this->link_icon_list;
        ...
        $data['mainContent'] = $this->load->view('distributor/purchase_order/purchase_order_edit', $data, true);
        $this->load->view("distributor/masterTemplateSmeMobile", $data);
    }
     ...
     * @return mixed
     */
    public function load_so_po_ides_by_supplier_customer()
    {
    ...
    {
        /*page navbar details*/
        $data['title'] = get_phrase('purchase_order_list');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('New_Purchase_Order');
        $data['link_page_url'] = $this->project . '/purchase_order';
        $data['link_icon'] = "<i class='ace-icon fa fa-plus'></i>";
        /*page navbar details*/
        $data['mainContent'] = $this->load->view('distributor/purchase_order/purchase_order_list', $data, true);
        $this->load->view('distributor/masterTemplate', $data);
    }
    ...
    {
        $data['title'] = get_phrase('Purchase Order View');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('New_Purchase_Order');
        $data['link_page_url'] = $this->project . '/purchase_order';
        $data['link_icon'] = $this->link_icon_add;
        $data['second_link_page_name'] = get_phrase('purchase_order Order List');
        $data['second_link_page_url'] = $this->project . '/purchase_order_list';
        $data['second_link_icon'] = $this->link_icon_list;
        $data['therd_link_icon'] = '<i class="fa fa-list"></i>';
        $data['third_link_page_name'] = get_phrase('Purchase_Order_Edit');
        $data['third_link_page_url'] = $this->project . '/purchase_order_edit/' . $id;
        ...
        $data['supplier'] = $this->Common_model->get_single_data_by_single_column('supplier', 'sup_id', $data['so_po_info']->supplier_id);
        ...
        $data['mainContent'] = $this->load->view('distributor/purchase_order/purchase_order_view', $data, true);
        $this->load->view('distributor/masterTemplate', $data);
...
//        }
    ...
    public function purchase_invoice_against_purchase_order_add()
    {
        $this->load->helper('sales_invoice_no_helper');
        ...
        if (isPostBack()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                $msg = 'Required field can not be empty';
                $this->session->set_flashdata('error', $msg);
                redirect(site_url($this->project . '/purchaseInvoiceAgainstPO/'));
            } else {
                ...
                $sales_inv['supplier_invoice_no'] = $so_id;
                $sales_inv['supplier_id'] = $supplier_id;
                ...
                $invoice_id = $this->Common_model->insert_data('purchase_invoice_info', $sales_inv);
                if ($payType == 2) {
                ...
                foreach ($_POST['so_po_product_row_id'] as $key => $value) {
                    ...
                    $sales_details_id = $this->Common_model->insert_data('purchase_details', $stock);
                    ...
                    $stockNewTable['last_purchase_price'] = 0;
        ...
        $data['title'] = get_phrase('purchases_order_edit');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('New_purchases_Order');
        $data['link_page_url'] = $this->project . '/purchase_order';
        $data['link_icon'] = $this->link_icon_add;
        $data['second_link_page_name'] = get_phrase('purchase_order_list');
        $data['second_link_page_url'] = $this->project . '/purchase_order_list';
        ...
        $data['mainContent'] = $this->load->view('distributor/purchase_order/purchase_invoice_against_purchase_order_add', $data, true);
        $this->load->view("distributor/masterTemplateSmeMobile", $data);
    }
...
//add custom filter here
    ...
    function getSupplierCurrentBalance($supplier_id = null)
    {
        ...
        $presentBalance = $this->Purchases_Model->getSupplierBalance($this->dist_id, $supplier_id);
