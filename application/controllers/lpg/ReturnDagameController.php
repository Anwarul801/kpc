...
//Inventory stock
    ...
    function salesReturnAdd()
    {
        ...
        if (isPostBack()) {
            ...
            $query = $this->db->field_exists('parent_stock_id', 'stock');
            if ($query != TRUE) {
                $this->load->dbforge();
                $fields = array(
                    'parent_stock_id' => array(
                        'constraint' => 11,
                        'unsigned' => TRUE,
                        'default' => '0',
                        'null' => TRUE
...
;";
            ...
            foreach ($sales_details_id as $a => $value) {
                ...
                $stockNewTable['parent_stock_id'] = $sales_invoice_id[$value];
        ...
        $menu  = check_parmission_by_user_role(109);
...
;";
            ...
            foreach ($purchase_details_id as $a => $value) {
                ...
                $stockNewTable = array();
                $stockNewTable['parent_stock_id'] = $purchase_return_invoice_id;;
                ...
                $stockNewTable['update_by'] = '';
                $stockNewTable['update_date'] = '';
                $stockNewTable['parent_stock_id'] = $purchase_invoice_id[$value];
