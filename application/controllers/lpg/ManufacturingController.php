...
class ManufacturingController extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->TypeDR = 1;
        $this->TypeCR = 2;
        $this->folderSub = 'distributor/inventory/manufacturer/';
    ...
    {
        $this->load->helper('manufacturing_no_helper');
        if (isPostBack()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                redirect(site_url($this->project . '/manufacturingAdd/'));
            } else {
                $this->load->helper('manufacturing_no_helper');
                ...
                $inv_adjustment_no = create_manufacturing_no();
                $this->db->trans_start();
                $data = array();
                ...
                // echo "<pre>";
                // print_r($inventory_adjustment_details);
                // exit;
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    redirect(site_url($this->project . '/manufacturingShow/'.$id));
                }
            }
        }
        $data['voucherID'] = create_manufacturing_no();
        $data['customerList'] = $this->Sales_Model->getCustomerList($this->dist_id);
        $data['title'] = get_phrase('Manufacturing Add');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('Manufacturing List');
        $data['link_page_url'] = $this->project . '/manufacturingList';
        ...
        $menu  = check_parmission_by_user_role(112);

        if($add == 0 || $menu == 0){
            $data['mainContent'] = $this->load->view('distributor/not_permisson_page', $data, true);
            $this->load->view('distributor/masterTemplate', $data);
        } else{
            $data['mainContent'] = $this->load->view('distributor/inventory/manufacturing/manufacturingAdd', $data, true);
            $this->load->view('distributor/masterTemplate', $data);
        }
    ...
    public  function  manufacturingList(){
        $data['title'] = get_phrase('manufacturing List');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('manufacturing Add');
        $data['link_page_url'] = $this->project.'/manufacturingAdd';
        $data['adjustmentBrand'] = $this->Common_model->getAllAdjustmentbrand();
        $data['branch'] = $this->Common_model->branchList();
        $data['mainContent'] = $this->load->view('distributor/inventory/manufacturing/manufacturingList', $data, true);
        $this->load->view('distributor/masterTemplate', $data);
    }

    function manufacturingShow($id)
    {
        ...
        $data['title'] = get_phrase('manufacturing View');
        $data['page_type'] = get_phrase('Configuration');
        $data['link_page_name'] = get_phrase('manufacturing List');
        $data['link_page_url'] = $this->project . '/manufacturingList';
        $data['link_icon'] = "<i class='fa fa-list'></i>";
        $data['mainContent'] = $this->load->view('distributor/inventory/manufacturing/manufacturingShow', $data, true);
        $this->load->view('distributor/masterTemplate', $data);
    }
    ...
    function manufacturingEdit($id)
    {
        ...
        $data['customerList'] = $this->Sales_Model->getCustomerList($this->dist_id);
        $data['title'] = get_phrase('manufacturing Update');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('manufacturing List');
        $data['link_page_url'] = $this->project . '/manufacturingList';
        $data['link_icon'] = "<i class='fa fa-list'></i>";
        $data['productCat'] = $this->Common_model->getPublicProductCat($this->dist_id);
        $data['branch'] = $this->Common_model->branchList();
        $data['mainContent'] = $this->load->view('distributor/inventory/manufacturing/manufacturingEdit', $data, true);
        $this->load->view('distributor/masterTemplate', $data);
    }
}