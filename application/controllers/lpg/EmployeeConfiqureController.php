...
class EmployeeConfiqureController extends CI_Controller
{
    ...
    public function employeeConfiquration()
    {
       ...
       if (isPostBack()) {
                ...
                $this->db->update('employeelabelfield', $Condition);
                 
                    foreach ($_POST['fieldName'] as $key => $value) {
                        unset($data);
                       $get_id = $this->Common_model->get_single_data_by_single_column('employeelabelfield', 'id', $value);
                ...
                $this->db->update_batch('employeelabelfield', $allAccess,'id');
        ...
        $data['EmployeeField'] = $this->Common_model->get_data_list('employeelabelfield', 'id', 'ASC');
    ...
    public function saveBankBookConfig()
    {
        ...
        if ($_POST['action_type'] == 'add') {
            ...
            $this->db->from("employeelabelfield");
            $this->db->where('employeelabelfield.fieldName', $fieldName);
            ...
            if ($number_of_result->number_of_result > 0) {
                ...
                $this->db->trans_start();
                $this->db->insert('employeelabelfield', $data);
                $this->db->trans_complete();
                if ($this->db->trans_status() === FALSE) {
                    echo '0';
                } else {
            ...
            $this->db->trans_start();
            $this->db->where('id', $id);
            $this->db->delete('employeelabelfield');
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                echo '0';
            } else {
    ...
    public function getconfig()
    {
        ...
        $EmployeeField = $this->Common_model->get_data_list('employeelabelfield', 'id', 'ASC');
