...
class SalesLpgController extends CI_Controller
{
    ...
    public function salesLpgInvoice_add($confirmId = null)
    {
        ...
        if (isPostBack()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $sales_inv['guarantorName'] = $this->input->post('guarantorName');
                $sales_inv['guarantorAddress'] = $this->input->post('guarantorAddress');
    ...
    //  exit;
        if($add == 0){
            ...
            if ($this->business_type != "LPG") {
                $data['mainContent'] = $this->load->view('distributor/sales/salesInvoiceMobile/sale_add_', $data, true);
            } else {
