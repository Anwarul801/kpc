...
class SalesDeliveryChallanController extends CI_Controller
{
    private $timestamp;
    private $admin_id;
    public $dist_id;
    ...
    public function sales_delivery_challan()
    {
        ...
        if (isPostBack()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $sales_inv['customer_invoice_no'] =$this->input->post('sdc_mrn_no');;
                $sales_inv['customer_id'] = $supplier_id;
                $sales_inv['payment_type'] = 2;

                $sales_inv['invoice_amount'] = $this->input->post('total_price');
                $sales_inv['delivery_address'] =$shipping_address;
                $sales_inv['delivery_date'] = $mrn_recived_date;
                ...
                $sales_inv['invoice_date'] = $mrn_recived_date;
                ...
                $sales_purchase_invoice_id= $this->Common_model->insert_data('sales_invoice_info', $sales_inv);



                $sdc_mrn_info = array();
                $sdc_mrn_info['form_id'] = 31;
                $sdc_mrn_info['sdc_mrn_no'] = $sdc_mrn_no=create_mrn_sdc_no('','SDC');
                $sdc_mrn_info['customer_id'] = $supplier_id;
                $sdc_mrn_info['supplier_id'] = 0;
                $sdc_mrn_info['shipping_address_mrn_sdc'] = $shipping_address;
                $sdc_mrn_info['sdc_delivery_date'] = $mrn_recived_date;
                ...
                $accountingMasterTable['Narration'] = 'SDC ';
                ...
                foreach ($_POST['so_po_product_row_id'] as $key => $value) {
                    ...
                    unset($stock);
                    $stock['sales_invoice_id'] =$sales_purchase_invoice_id;
                    $stock['customer_id'] = $supplier_id;
                    ...
                    $stockNewTable['price_in'] = $unit_price;
                    ...
                    $accountingDetailsTable['GR_CREDIT'] = $unit_price*$quantity;
                ...
                $condtion = array(
                    'parent_name' => "Item Delivery But Bill Pending",
                    //'related_id_for' => 2,
                    'is_active' => "Y",
                );
                ...
                if (empty($Item_received_but_bill_pending)) {
                    ...
                    $msg = 'Please Create A ledger Under Customer Recivable Named Item Delivery But Bill Pending';
                ...
                /*Supplier Payable =======>34*/
                $accountingDetailsTable['Accounts_VoucherMst_AutoID'] = $accountingVoucherId;
                $accountingDetailsTable['TypeID'] = '1';//Cr
                $accountingDetailsTable['CHILD_ID'] = $Item_received_but_bill_pending->id;//Supplier_Payables'34';
                $accountingDetailsTable['GR_DEBIT'] = $this->input->post('total_price');
                $accountingDetailsTable['GR_CREDIT'] = 0;
                $accountingDetailsTable['Reference'] = 'Item Received But Bill Pending';

                $accountingDetailsTable['invoice_no'] = $sdc_mrn_no . "/" . $invoice_no;
                $accountingDetailsTable['invoice_id'] = $sales_purchase_invoice_id;
                $accountingDetailsTable['for'] = 5;
                ...
                if ($this->db->trans_status() === FALSE) {
                    $msg = 'SDC' . ' ' . $this->config->item("save_error_message");
                    $this->session->set_flashdata('error', $msg);
                    redirect(site_url($this->project . '/sales_delivery_challan/'));
                } else {
                    $msg = 'SDC ' . ' ' . $this->config->item("save_success_message");
                    $this->session->set_flashdata('success', $msg);
                    redirect(site_url($this->project . '/sales_delivery_challan/'));
        ...
        $data['second_link_page_name'] = get_phrase('sales_delivery_challan List');
        $data['second_link_page_url'] = $this->project . '/sales_delivery_challan_list';
...
//add custom filter here
    ...
    {
        /*page navbar details*/
        $data['title'] = get_phrase('sdc_list');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('add_sdc');
        $data['link_page_url'] = $this->project . '/sales_delivery_challan';
        $data['link_icon'] = "<i class='ace-icon fa fa-plus'></i>";
        /*page navbar details*/
        $data['mainContent'] = $this->load->view('distributor/sales_order/sdc_list', $data, true);
...
//add custom filter here
    ...
    public function material_received_note_prepare_bill($id)
    {
        ...
        if (isPostBack()) {
            ...
            if ($this->form_validation->run() == FALSE && $id == '') {
                ...
                $supplier_id = $sdc_mrn_info->customer_id;
                ...
                $ac_tb_accounts_voucherdtl['Reference'] = 'Customer Recivable for '.$sdc_mrn_info->customer_invoice_no;
                ...
                if($this->input->post('partialPayment')>0){
                    ...
                    $accountingDetailsTable['Reference'] = 'Customer Paid';
                    //$accountingDetailsTable['invoice_no'] = 'Customer Paid';
                    $accountingDetailsTable['IsActive'] = 1;
                    $accountingDetailsTable['Created_By'] = $this->admin_id;
                    $accountingDetailsTable['Created_Date'] = $this->timestamp;
                    ...
                    $purchase_invoice_info_condition = array(
                        'sales_invoice_id' => $purchase_sales_invoice_id,
                    );
                    $purchase_invoice_info = array();
                    $purchase_invoice_info['paid_amount'] =  $this->input->post('partialPayment');
                    $this->Common_model->save_and_check('sales_invoice_info', $purchase_invoice_info, $purchase_invoice_info_condition);
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    $msg = 'Material Received Note ' . ' ' . $this->config->item("save_success_message");
                    $this->session->set_flashdata('success', $msg);
                    redirect(site_url($this->project . '/view_sdc_bill/'.$id));
                    //redirect(site_url($this->project . '/viewLpgCylinder/' . $sales_purchase_invoice_id));
                }
            }
        }
        ...
        $data['title'] = get_phrase('SDC prepare_bill');
        $data['page_type'] = get_phrase($this->page_type);
        $data['second_link_page_name'] = get_phrase('SDC List');
        $data['second_link_page_url'] = $this->project . '/sales_delivery_challan_list';
        $data['second_link_icon'] = $this->link_icon_list;
        ...
        $data['customerInfo'] = $this->Common_model->get_single_data_by_single_column('customer', 'customer_id', $data['sdc_mrn_info']->customer_id);
        ...
        $data['mainContent'] = $this->load->view('distributor/sales_order/sdc_prepare_bill', $data, true);
        $this->load->view("distributor/masterTemplateSmeMobile", $data);
    }

    function view_sdc_bill($id)
    {

        $data['title'] = get_phrase('SDC prepare_bill');
        $data['page_type'] = get_phrase($this->page_type);
        $data['second_link_page_name'] = get_phrase('SDC_list');
        $data['second_link_page_url'] = $this->project . '/sales_delivery_challan_list';
        $data['second_link_icon'] = $this->link_icon_list;
    ...
    // echo $this->db->last_query();exit;
        ...
        $moneyreceit_condtion = array(
            'customerid' => $data['sdc_mrn_info']->customer_id,
            'receiveType' => "1",
        ...
        $data['mainContent'] = $this->load->view('distributor/sales_order/view_sdc_bill', $data, true);
        $this->load->view("distributor/masterTemplateSmeMobile", $data);

    }

    public function sdc_view($id)
    {
        $data['title'] = get_phrase('SDC View');
        $data['page_type'] = get_phrase($this->page_type);
        $data['link_page_name'] = get_phrase('SDC_add');
        $data['link_page_url'] = $this->project . '/sales_delivery_challan';
        $data['link_icon'] = $this->link_icon_add;
        $data['second_link_page_name'] = get_phrase('SDC_list');
        $data['second_link_page_url'] = $this->project . '/sales_delivery_challan_list';
    ...
    /*  echo $this->db->last_query();
       ...
       exit;*/
        ...
        $data['mainContent'] = $this->load->view('distributor/sales_order/sdc_view', $data, true);
        $this->load->view('distributor/masterTemplate', $data);
    }
