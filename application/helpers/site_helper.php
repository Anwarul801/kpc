...
if (!function_exists('get_settings')) {
    function get_settings($key = '',$status=1)
    {
        $CI =& get_instance();
        $CI->load->database();
        ...
        if($status==1){
            $CI->db->where('is_active', $status);
        }
        $result_settings = $CI->db->get('settings')->row('s_value');
...
if (!function_exists('get_settings_status')) {
    function get_settings_status($key = '')
    {
        $CI =& get_instance();
        $CI->load->database();
        ...
        $result_settings = $CI->db->get('settings')->row('is_active');
...
if (!function_exists('check_parmission_by_user_role')) {
    ...
    if (!function_exists('get_number_of_property_in_show_list')) {
        function get_number_of_property_in_show_list( $is_show = 1)
        {
            $CI =& get_instance();
            $CI->load->database();


            $CI->db->select('count(*) as number_of_property');
            $CI->db->from('product_property');
            $CI->db->where('is_show',$is_show);
            $CI->db->where_in('property_id',array(1,2,3,4,5));
         

            $sql = $CI->db->get();
            $result = $sql->row();
            if (!empty($result)) {
                return $result->number_of_property;
            } else {
                return 0;
            }
