...
class ServerFilterModel extends CI_Model
{
    ...
    private function _get_productCat_datatables_query()
    {
        ...
        $this->db->select("productcategory.*,admin.name");
        $this->db->from("productcategory");
        $this->db->join('admin', 'admin.admin_id=productcategory.updated_by');
        $this->db->group_start();
        $this->db->where('dist_id', $this->dist_id);
        $this->db->or_where('dist_id', 1);
        $this->db->group_end();
    ...
    private function _get_datatables_unit_query()
    {
     ...
     function get_purreturn_datatables()
    {
     ...
     private function _get_purchase_return_datatables_query()
    {
        ...
        $this->db->join('supplier', 'supplier.sup_id=return_info.sup_cus_id');
        ...
        $this->db->where('return_info.is_active', 'Y');
            ...
            $this->db->where('return_info.return_type ', '1');
    ...
    {
        $this->db->select("productcategory.title,brand.brandName,product.product_code,product.productName,product.purchases_price,product.salesPrice,product.retailPrice,product.status,product.dist_id,product.product_id,unit.unitTtile,admin.name");
        $this->db->from($this->table);
        ...
        $this->db->join('admin', 'admin.admin_id=product.updated_by');
    ...
    function get_po_datatables()
    {
     ...
     private function _get_po_datatables_query()
    {
        $this->db->select("so_po_info.id,
        so_po_info.so_po_no,
        so_po_info.status,
        ...
        ");
        $this->db->from("so_po_info");
        $this->db->join('supplier', 'supplier.sup_id=so_po_info.supplier_id','left');
        $this->db->join('so_po_productes', 'so_po_productes.so_po_id=so_po_info.id','left');

        // $this->db->where('sales_invoice_info.dist_id', $this->dist_id);
        $this->db->where('so_po_info.is_active', 'Y');
        $this->db->where('so_po_info.form_id =', '8');
        ...
        /*if (!empty($this->input->post('property_3'))) {
            $this->db->like("GROUP_CONCAT(so_po_productes.one_line_narration SEPARATOR ', ')as one_line_narration", $this->input->post('one_line_narration'), 'both');
        }*/
        $this->db->group_by("so_po_info.id");
        $i = 0;
        foreach ($this->column_search as $item) { // loop column
    ...
    function get_mrn_datatables()
    {
        $this->_get_mrn_datatables_query();
        if ($_POST['length'] != -1)
            $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();
    }
    private function _get_mrn_datatables_query()
    {
        $this->db->select("sdc_mrn_info.id,
        sdc_mrn_info.sdc_mrn_no,
        sdc_mrn_info.status,
        sdc_mrn_info.narration,
        
        DATE_FORMAT(sdc_mrn_info.mrn_recived_date, '%b %e, %Y') as mrn_recived_date,
       
        supplier.sup_id,
        supplier.supName,
        supplier.supID,admin.name,
        ...
        GROUP_CONCAT(sdc_mrn_products.property_3 SEPARATOR ', ') as property_3,
        GROUP_CONCAT(sdc_mrn_products.property_4 SEPARATOR ', ') as property_4,
        GROUP_CONCAT(sdc_mrn_products.property_5 SEPARATOR ', ')as property_5
       
        ");
        $this->db->from("sdc_mrn_info");
        $this->db->join('supplier', 'supplier.sup_id=sdc_mrn_info.supplier_id','left');
        ...
        $this->db->where('sdc_mrn_info.form_id =', '32');
        //$this->db->where('so_po_info.invoice_for !=', '3');

        if (!empty($this->input->post('property_1'))) {
            $this->db->like('sdc_mrn_products.property_1', $this->input->post('property_1'), 'both');
        }
        if (!empty($this->input->post('property_2'))) {
            $this->db->like('sdc_mrn_products.property_2', $this->input->post('property_2'), 'both');
        }
        if (!empty($this->input->post('property_3'))) {
            $this->db->like('sdc_mrn_products.property_3', $this->input->post('property_3'), 'both');
        }
        /*if (!empty($this->input->post('property_3'))) {
            $this->db->like("GROUP_CONCAT(so_po_productes.one_line_narration SEPARATOR ', ')as one_line_narration", $this->input->post('one_line_narration'), 'both');
        }*/
        //$this->db->group_by("sdc_mrn_products.id");
        ...
        if (isset($_POST['order'])) { // here order processing
            ...
            //$order = $this->order;
            $this->db->order_by('sdc_mrn_info.id', 'desc');
            $this->db->order_by('sdc_mrn_info.mrn_recived_date', 'desc');
        }
    }

    public function count_all_mrn()
    {
        $this->db->from($this->table);
        $this->db->where('is_active', "Y");
    ...
    private function _get_purchases_cylinder_datatables_query()
    {
        ...
        $this->db->join('admin', 'admin.admin_id=purchase_invoice_info.insert_by');
        //$this->db->where('purchase_invoice_info.dist_id', $this->dist_id);
    ...
    private function _get_warranty_receipt_voucher_list_query()
    {
        ...
        $this->db->select('purchase_invoice_info.purchase_invoice_id,purchase_invoice_info.invoice_no,purchase_invoice_info.invoice_amount,
        purchase_invoice_info.narration,purchase_invoice_info.invoice_date,
        purchase_invoice_info.paid_amount,supplier.supID,supplier.supName,supplier.sup_id,supplier.sup_id,admin.name,
    ...
    private function _get_sup_datatables_query()
    {
        ...
        //'moneyreceit.date', 'moneyreceit.receitID', 'moneyreceit.moneyReceitid','moneyreceit.totalPayment','moneyreceit.checkStatus','moneyreceit.paymentType', 'customer.customerID', 'customer.customerName'
        $this->db->select("supplier.*,admin.name");
    ...
    function count_filtered_sales_order()
    {
        $this->_get_so_po_datatables_query();
        $query = $this->db->get();
        return $query->num_rows();
    }
    function count_filtered_purchase_order()
    {
        $this->_get_po_datatables_query();
        $query = $this->db->get();
        return $query->num_rows();
    ...
    public function count_all_sales_order()
    {
        $this->db->from($this->table);
        //$this->db->where('dist_id', $this->dist_id);
     ...
     public function count_all_purchases_order()
    {
        $this->db->from($this->table);
        $this->db->where('is_active', "Y");
    ...
    function get_sdc_datatables()
    {
        $this->_get_sdc_datatables_query();
        if ($_POST['length'] != -1)
            $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();
    }
    private function _get_sdc_datatables_query()
    {
        $this->db->select("sdc_mrn_info.id,
        sdc_mrn_info.sdc_mrn_no,
        sdc_mrn_info.status,
        ...
        DATE_FORMAT(sdc_mrn_info.mrn_recived_date, '%b %e, %Y') as mrn_recived_date,
        DATE_FORMAT(sdc_mrn_info.sdc_delivery_date, '%b %e, %Y') as sdc_delivery_date,
    ...
    public function count_all_sdc()
    {
        $this->db->from($this->table);
        $this->db->where('is_active', "Y");
        $this->db->where('form_id', 31);
        return $this->db->count_all_results();
    }
    function count_filtered_sdc()
    {
        $this->_get_sdc_datatables_query();
        $query = $this->db->get();
        return $query->num_rows();
