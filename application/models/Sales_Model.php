...
$query="SELECT
	...
	sales_invoice_info.invoice_date,
	sales_invoice_info.sales_invoice_id,
	(sales_product.sales_product_price + sales_invoice_info.transport_charge + sales_invoice_info.loader_charge-IFNULL(sales_invoice_info.discount_amount,0))AS invoice_amount,
	(sales_product.sales_product_price + sales_invoice_info.transport_charge + sales_invoice_info.loader_charge-IFNULL(sales_invoice_info.discount_amount,0))-IFNULL(paid_against_invoice.paid_amount,0) AS amount,
	IFNULL(paid_against_invoice.paid_amount,0) as paid_amount,
  ...
  FROM inventory_adjustment_details iad  LEFT OUTER JOIN
    ...
    function getCylinderInOutResult2($distId, $invoiceId, $formId)
    {
       ...
       // $this->db->where('stock.dist_id', $distId);
        //type 24 means cylinder receive,type 23 means cylinder out
        $this->db->where('stock.form_id', $formId);
        $this->db->where('stock.invoice_id', $invoiceId);
        $cylinderInOutResult = $this->db->get()->result();
        return $cylinderInOutResult;
    }
