				...
				UNION ALL
					...
					UNION ALL
						...
						UNION ALL
							...
							LEFT JOIN ac_account_ledger_coa alc ON alc.id=clild_ids.CHILD_ID";
        $query.=" where alc.status=1 ";
...
ORDER BY acl.parent_id ASC";
    ...
    function getAllsalaryList(){
        ...
        $this->db->where('salary_info.voucherType', '1');
    ...
    function getAllsalaryByDateList(){
        $this->db->select("salary_info.*, sum(netPayAmount) as total ,salary_info.date,salary_info.voucherNo,salary_info.year,salary_info.month");
        $this->db->from("salary_info");
    ...
    function getAllsalaryVoucherReport(){
    ...
    function getAllEmployee($employee=null,$department=null,$designation=null){
        if ($employee == "all" &&  $department ==NULL && $designation==NULL) {
        ...
        elseif ($employee == "all" &&  $department !==NULL && $designation==NULL) {
            $this->db->select("employee.*,employee.salaryType,employee.name,tb_department.DepartmentName,tb_designation.DesignationName,employee.salary");
            $this->db->from("employee");
        ...
        elseif ($employee == "all" &&  $department ==NULL && $designation !==NULL) {
            ...
            $this->db->where('employee.designation', $designation);
        ...
        elseif ($employee == "all" &&  $department !==NULL && $designation !==NULL) {
            ...
            $result = $this->db->get()->result();
            return $result;
        }elseif ($employee !== NULL &&  $department !==NULL && $designation ==NULL) {
            $this->db->select("employee.*,employee.salaryType,employee.name,tb_department.DepartmentName,tb_designation.DesignationName,employee.salary");
            $this->db->from("employee");
            $this->db->join('tb_department', 'tb_department.DepartmentID=employee.department','left');
            $this->db->join('tb_designation', 'tb_designation.DesignationID=employee.designation','left');
            ...
            $this->db->where('employee.id', $employee);
    ...
    function getMonthlySummery($month=null,$year=null){

        if ($month=="all") {
            $this->db->select("month,year,sum(salary_info.netPayAmount) as dep,tb_department.DepartmentName");
            $this->db->from("salary_info");
            $this->db->join('employee', 'employee.id=salary_info.employeeID','left');
            $this->db->join('tb_department', 'tb_department.DepartmentID=salary_info.departmentID','left');
            $this->db->group_by("salary_info.departmentID");
            $this->db->group_by("salary_info.month");
            $this->db->group_by("salary_info.year");
            $this->db->where('salary_info.year', $year);
            $this->db->where('salary_info.voucherType', '1');
